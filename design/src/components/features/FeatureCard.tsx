...
const FeatureCard = ({ gradient, children, id }: FeatureCardProps) => {
  ...
  return (
    ...
    >
      <div
        className={`gradient absolute inset-0 origin-bottom-left rounded-2xl bg-gradient-to-br`}
      >
        {children}
        <button
          onClick={() => setFullscreenFeature(id)}
          className=" show-me-btn bg-black text-white absolute bottom-6 right-6 py-2 px-4 rounded-xl shadow-lg"
        >
          Show More
        </button>
      </div>
...
export const Todo = ({ id }: CardProps) => {
  return (
    <FeatureCard id={id} gradient="from-[#f7f0ff] to-[#a78ae7]">
      <span>hello</span>
    </FeatureCard>
  );
};
...
export const Music = ({ id }: CardProps) => {
  ...
  return (
    <FeatureCard id={id} gradient="from-[#f5fff7] to-[#adf8ff]">
      <img
        className={`absolute left-[10%] top-[10%] w-[20%] shadow-lg rounded-xl ${
      ...
      />
      <img
        className={`absolute left-[70%] top-[20%] w-[25%] shadow-lg rounded-xl ${
          isFullscreen ? "scale-0" : "scale-100"
        }`}
        src="/song-2.webp"
        alt=""
      />
      <img
        className={`absolute left-[20%] top-[60%] w-[30%] shadow-lg rounded-xl  ${
          isFullscreen ? "scale-0" : "scale-100"
        }`}
        src="/song-3.webp"
        alt=""
      ...
      {!isFullscreen && (
        <motion.div
          ...
          className="bg-[#1bd761] p-3 absolute left-[40%] top-32 w-24  h-48 rounded-[96px] shadow-lg"
        >
...
export const Links = ({ id }: CardProps) => {
  return (
